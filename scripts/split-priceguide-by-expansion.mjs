...
async function main() {
  ...
  for (const f of prodFiles) {
    ...
    if (!meta) continue;
    if (/singles?/i.test(f)) {
      if (!latestSinglesDate || meta.date > latestSinglesDate) { latestSingles = f; latestSinglesDate = meta.date; }
    } else {
      if (!latestNonsinglesDate || meta.date > latestNonsinglesDate) { latestNonsingles = f; latestNonsinglesDate = meta.date; }
    }
  }
  ...
  for (const p of nonsArr)    pushProduct(p); // nur als Fallback

  const codeGuessForExpansion = new Map(
    [...codeHintsByExpansion.entries()].map(([exp, list]) => {
      const freqMap = list.reduce((a,c)=> (a[c]=(a[c]||0)+1, a), {});
      const best = Object.entries(freqMap).sort((a,b)=> b[1]-a[1])[0][0];
      return [exp, best];
    })
  );
  ...
  for (const name of existing) {
    if (/^price_guide_\d+_(?:[A-Z]{2}-\d{2}_exp-\d+|exp-\d+)_\d{4}-\d{2}-\d{2}\.json$/.test(name) || name === 'index.json') {
  ...
  for (const { file, langOrMarket } of todaysGuides) {
    ...
    // Schreiben pro Expansion (eindeutiger Dateiname!)
